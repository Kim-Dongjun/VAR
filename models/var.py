    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums):   # si: i-th segment
            ...
            for b in self.blocks:
                ...
                print("x: ", x.shape) # [2B, pn**2, hidden_dim]
            ...
            print("idx_Bl: ", idx_Bl.shape) # [B, pn**2]
            if not more_smooth: # this is the default case
                h_BChw = self.vae_quant_proxy[0].embedding(idx_Bl)   # B, l, Cvae
            else:   # not used when evaluating FID/IS/Precision/Recall
                gum_t = max(0.27 * (1 - ratio * 0.95), 0.005)   # refer to mask-git
                h_BChw = gumbel_softmax_with_rng(logits_BlV.mul(1 + ratio), tau=gum_t, hard=False, dim=-1, rng=rng) @ self.vae_quant_proxy[0].embedding.weight.unsqueeze(0)
            print("h_BChw: ", h_BChw.shape) # [B, pn**2, Ch]
            h_BChw = h_BChw.transpose_(1, 2).reshape(B, self.Cvae, pn, pn)
            print("h_BChw: ", h_BChw.shape) # [B, Ch, pn, pn]
            f_hat, next_token_map = self.vae_quant_proxy[0].get_next_autoregressive_input(si, len(self.patch_nums), f_hat, h_BChw)
            print("f_hat: ", f_hat.shape) # [B, Ch, h, w]
            print("next_token_map: ", next_token_map.shape) # [B, Ch, next_pn, next_pn]
